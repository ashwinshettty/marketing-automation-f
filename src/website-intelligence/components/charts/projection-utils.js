...
function resolveIntervalMs(sourceData, xDataKey, startIndex) {
  return intervalFromAdjacentRows(sourceData, xDataKey, startIndex) ?? intervalFromSeriesSpan(sourceData, xDataKey) ?? 864e5;
}
...
function computeProjectionAnchorTangentSlope(sourceData, seriesKey, xDataKey = "date", startIndexProp) {
  if (sourceData.length < 2) {
    return 0;
  }
...
function buildHorizontalTangentBezierPath(x0, y0, x1, y1, tension = 0.45) {
  const dx = x1 - x0;
  if (Math.abs(dx) < 1e-6) {
...
function buildProjectionPath(options) {
  const {
    sourceData,
    seriesKey,
    xDataKey = "date",
